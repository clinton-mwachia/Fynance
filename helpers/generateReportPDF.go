...
func GenerateReportPDF(w fyne.Window, records []models.Report, filename string) {
	dir := "reports"
	...
	pdf.SetFont("Arial", "", 12)
	pdf.CellFormat(190, 8, title, "0", 1, "C", false, 0, "")
	pdf.CellFormat(190, 8, fmt.Sprintf("Email: %s | Phone: %s", "example@gmail.com", "0746646331"), "0", 1, "C", false, 0, "")
	pdf.Ln(3)

	// Title
	pdf.SetFont("Arial", "B", 12)
	pdf.CellFormat(190, 10, fmt.Sprintf("%s as at %s", title, time.Now().Format("02-01-2006 15:04:05")), "", 1, "C", false, 0, "")
	pdf.Ln(3)
	...
	defaultFileName := fmt.Sprintf("%s/%s_%s.pdf", dir, filename, timestamp)

	if err := pdf.OutputFileAndClose(defaultFileName); err != nil {
		dialog.ShowError(err, w)
		return
	}
	dialog.ShowInformation("Success", fmt.Sprintf("%s PDF Generated", title), w)
}
