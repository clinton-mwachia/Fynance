...
func DailyReport(window fyne.Window) *fyne.Container {
	...
	loadReports := func() {
		...
		go func() {
			reports, _ = utils.GetDailyReport(window)

			fyne.Do(func() {
				dailyReportList.Refresh()
			})
	...
	dailyReportList = widget.NewList(
		func() int {
			return len(reports)
		},
	...
	exportToCSVBtn := widget.NewButton("Export to CSV", func() {
		ExportDailyCSVReport(window)
	})

	exportToPDFBtn := widget.NewButton("Export to PDF", func() {
		showDailyReportPDFDialog(window)
	})
	...
	listContainer := container.NewBorder(titleRow, nil, nil, nil, dailyReportList, noResultsLabel)
...
func ExportDailyCSVReport(window fyne.Window) {
	records, err := utils.GetDailyReport(window)
	...
	if err != nil {
		...
		if len(records) != 0 {
			...
			progressDialog := dialog.NewCustom("Exporting Daily Reports", "Cancel", progress, window)
			progressDialog.Show()

			go func() {
				file, err := os.Create("daily_report.csv")
				if err != nil {
					dialog.ShowError(err, window)
					return
				}
				...
				progressDialog.Hide()
				dialog.ShowInformation("Export Successful", "Records have been exported to daily_report.csv", window)
			}()
		} else {
...
// show report dialog
func showDailyReportPDFDialog(w fyne.Window) {
	records, _ := utils.GetDailyReport(w)

	if len(records) == 0 {
		dialog.ShowInformation("Daily Report", "NO DATA TO GENERATE REPORT", w)
	} else {
